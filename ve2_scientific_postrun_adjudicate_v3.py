...
"""Prospective v3 post-run adjudicator.

V3 preserves v1 scientific adjudication semantics and v2 accounting, while
repairing a pre-science identity-type defect: the frozen frontier constants are
Git blob SHA-1 identities, not raw-file SHA-256 values.
"""
...
def main() -> None:
    ...
    # These answer-bearing files are not opened or parsed before the complete
    # sixteen-stage raw seal has passed above. Their frozen identities are Git
    # blob identities, matching the public pre-result authority.
    t01_path = Path(args.t01_frontier)
    t02_path = Path(args.t02_frontier)
    base.require(git_blob_sha(t01_path) == base.T01_FRONTIER_SHA, "t01-frontier-git-blob")
    base.require(git_blob_sha(t02_path) == base.T02_FRONTIER_SHA, "t02-frontier-git-blob")
    ...
    result = {
        "schema": "replaymark.ve2.independent-postrun-adjudication.v3",
        "status": "PASS" if t01["status"] == "PASS" and t02["status"] == "PASS" else "FAIL",
        "raw_seal_sha256": seal["seal_sha256"],
        "frontier_identities": {
            "t01_git_blob": base.T01_FRONTIER_SHA,
            "t02_git_blob": base.T02_FRONTIER_SHA,
        },
        "t01": t01,
        "t02": t02,
        "accounting": {
