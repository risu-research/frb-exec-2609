...
"""VE2 version-spanning Home Assistant boundary microkernel v2.

Prospective remediation of E0Q-v1 observer scheduling only.  T01/T02 state,
frontier labels, Better Thermostat actions, and scientific expectations are
intentionally absent.  Home Assistant-native @callback handlers keep event
observation on the event-loop callback path.  A capture epoch plus observer-
local monotonic sequence establishes causal observation order independently of
executor scheduling.
"""
...
SERVICE_EVENT_SCHEMA = "replaymark.ve2.boundary-service-event.v2"
STATE_EVENT_SCHEMA = "replaymark.ve2.boundary-state-event.v2"
CAPTURE_EPOCH_SCHEMA = "replaymark.ve2.boundary-capture-epoch.v2"
COMPLETION_SCHEMA = "replaymark.ve2.completed-boundary.v2"
...
class BoundaryObserver:
    ...
    def arm_epoch(self) -> dict[str, Any]:
        if not self._unsubs:
            raise RuntimeError("cannot arm uninstalled observer")
        value = {
            "schema": CAPTURE_EPOCH_SCHEMA,
            "clock_domain": CLOCK_DOMAIN,
            "armed_ns": time.perf_counter_ns(),
            "state_start_index": len(self.state_events),
            "service_start_index": len(self.service_events),
            "sequence_floor": self._sequence,
        }
        validate_capture_epoch(value)
        return value
...
def validate_capture_epoch(value: dict[str, Any]) -> dict[str, Any]:
    if set(value) != {
        "schema", "clock_domain", "armed_ns", "state_start_index",
        "service_start_index", "sequence_floor",
    }:
        raise ValueError("capture-epoch-field-set")
    if value["schema"] != CAPTURE_EPOCH_SCHEMA or value["clock_domain"] != CLOCK_DOMAIN:
        raise ValueError("capture-epoch-schema-or-clock")
    for key in ("armed_ns", "state_start_index", "service_start_index", "sequence_floor"):
        if not isinstance(value[key], int) or value[key] < 0:
            raise ValueError("capture-epoch-nonnegative-int:" + key)
    return value
...
def validate_state_event(value: dict[str, Any]) -> dict[str, Any]:
    expected = {
        "schema", "clock_domain", "sequence", "t_ns", "entity_id", "old_state",
        "new_state", "context_id", "context_parent_id",
    }
    ...
    if not isinstance(value["sequence"], int) or value["sequence"] <= 0:
        raise ValueError("state-event-sequence")
...
def validate_service_event(
    ...
    *,
    ...
    expected = {
        "schema", "clock_domain", "sequence", "t_ns", "domain", "service",
        "service_data", "context_id", "context_parent_id",
    }
    if set(value) != expected:
    ...
    if not isinstance(value["sequence"], int) or value["sequence"] <= 0:
        raise ValueError("service-event-sequence")
    if not isinstance(value["t_ns"], int):
...
def context_bound(value: dict[str, Any], invocation_context_id: str) -> bool:
    ...
    return value["context_id"] == invocation_context_id or value["context_parent_id"] == invocation_context_id
...
def select_parent_state_event(
    ...
    *,
    ...
    validate_capture_epoch(epoch)
    matches = []
    for event in events[epoch["state_start_index"]:]:
    ...
    parent = copy.deepcopy(matches[0])
    require_event_after_epoch(epoch, parent)
    return parent
...
def qualifying_service_events(
    ...
    *,
    ...
    validate_capture_epoch(epoch)
    out = []
    for event in events[epoch["service_start_index"]:]:
        validate_service_event(event)
        if event["domain"] == domain and event["service"] == service and context_bound(event, invocation_context_id):
            require_event_after_epoch(epoch, event)
...
def require_event_after_epoch(epoch: dict[str, Any], event: dict[str, Any]) -> None:
    validate_capture_epoch(epoch)
    if event.get("sequence", -1) <= epoch["sequence_floor"]:
        raise ValueError("event-sequence-not-after-arm")
    if event.get("t_ns", -1) < epoch["armed_ns"]:
        raise ValueError("event-time-before-arm")


def require_positive_boundary(
    epoch: dict[str, Any],
    parent: dict[str, Any],
    event: dict[str, Any],
    ...
    if parent["t_ns"] > event["t_ns"]:
        raise ValueError("service-time-not-after-parent")
    if parent["t_ns"] > completion["closed_ns"] or event["t_ns"] > completion["closed_ns"]:
        raise ValueError("positive-event-after-completion")


def require_absence_boundary(
    epoch: dict[str, Any],
    parent: dict[str, Any],
    qualifying_events: Iterable[dict[str, Any]],
    completion: dict[str, Any],
) -> None:
    validate_capture_epoch(epoch)
    validate_state_event(parent)
    validate_completion(completion)
    require_event_after_epoch(epoch, parent)
    require_exact_cardinality(qualifying_events, 0)
    if parent["t_ns"] > completion["closed_ns"]:
        raise ValueError("absence-parent-after-completion")
