...
def source_identity(repo: pathlib.Path, role: str, dst_root: pathlib.Path) -> dict[str, Any]:
    ...
    source_dir = dst_root / role.lower()
...
def image_identity(tag: str) -> dict[str, Any]:
    ...
    if not matching:
        raise RuntimeError(("no-repodigest", ref, repo_digests))
...
def inside_probe(image: dict[str, Any], source: dict[str, Any], inside_script: pathlib.Path, work: pathlib.Path) -> dict[str, Any]:
    result_path = work / ("inside-" + source["role"].lower() + "-" + image["tag"].replace(".", "_") + ".json")
    ...
    cmd = ["docker","run","--rm","--network","none","--read-only","--tmpfs","/tmp:rw,nosuid,nodev,size=64m",
           ...
           "--volume",f'{source["source_dir"]}:/probe:ro',"--volume",f'{inside_script.resolve()}:/ve2_inside_probe.py:ro',
           "--volume",f'{result_path.resolve()}:/result.json',"--entrypoint","python",image["reference"],"/ve2_inside_probe.py"]
    proc = run(cmd, check=False)
    try:
        result = load_json(result_path)
    except Exception as exc:
        result = {"status":"FAIL","error":f"unreadable-inside-result:{exc}"}
...
def main() -> int:
    ...
    work=pathlib.Path(args.work);out=pathlib.Path(args.out);work.mkdir(parents=True,exist_ok=True);out.parent.mkdir(parents=True,exist_ok=True)
    result={"schema":"replaymark.ve2.runtime-discovery-result.v1","status":"FAIL","scientific_result_opened":False,"frontier_result_seen":False,"ve2_home_assistant_scientific_cells":0,"ve2_replaymark_scientific_cells":0,"transition_fixture_executed":False,"historical_action_dispatched":False}
    try:
        constitution=load_json(pathlib.Path(CONSTITUTION));candidates=load_json(pathlib.Path(CANDIDATES));authority=load_json(pathlib.Path(AUTHORITY))
        ...
        srcroot=work/"sources";srcroot.mkdir()
    ...
    print(json.dumps({"status":result["status"],"science":0,"result_sha256":sha256_bytes(payload.encode()),"selected_tags":{k:(v["tag"] if isinstance(v,dict) and "tag" in v else None) for k,v in (result.get("selected") or {}).items()}},sort_keys=True))
