...
def main():
    result = {
        "schema": "replaymark.ve2.runtime-inside-probe.v3",
        "status": "FAIL",
        "probe_definition": "PACKAGE_AND_CLIMATE_ENTRYPOINTS",
    ...
    try:
        ...
        # The v3 obligation follows the integration's actual load surface across all
        # frozen eras. A standalone const.py is historical/optional, not an entrypoint.
        const_path = os.path.join(comp, "const.py")
        result["standalone_const_module_present"] = os.path.isfile(const_path)
        imported = []
        for module in (
